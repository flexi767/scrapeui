...
export async function POST(request: Request) {
  ...
  const listingId = typeof payload?.listingId === 'number' ? payload.listingId : Number.parseInt(String(payload?.listingId ?? ''), 10);
  ...
  const id = createSavedSearch(listingId, fields);
  const detail = await getSavedSearchDetail(id);
  if (!detail) {
    return NextResponse.json({ error: 'Saved search created but could not be loaded' }, { status: 500 });
  }
