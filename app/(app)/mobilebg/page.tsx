...
export default function MobileBgPage() {
  ...
  const cards = [
    ...
    { label: 'Backed up listings', value: summary.backups },
    { label: 'Stored images', value: summary.images },
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-400">
          Database-backed backup, edit-form, and repost artifacts for mobile.bg. Images remain stored locally and are served through scrapeui.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-3">
        <section className="rounded-lg border border-gray-700 bg-gray-900/40">
          <div className="flex items-center justify-between border-b border-gray-700 px-4 py-3">
            <h2 className="text-sm font-medium text-gray-200">Recent Backup Runs</h2>
            <Link href="/mobilebg/backups" className="text-xs text-blue-300 hover:text-blue-200">Open backups</Link>
          </div>
          <div className="divide-y divide-gray-800">
            {runs.length === 0 ? (
              <div className="px-4 py-6 text-sm text-gray-500">No backup runs yet.</div>
            ) : runs.map((run) => (
              <div key={run.id} className="px-4 py-3 text-sm">
                ...
                <div className="mt-1 text-xs text-gray-400">
                  {run.listings_count} listings • {run.images_count} images • {formatDate(run.started_at || run.created_at)}
                </div>
              </div>
            ))}
          </div>
